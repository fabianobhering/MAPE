...
void queuesAddPacket(t_queues * queues, t_packet * packet, int node) {
	...
	//printf("Queue Size: %d	%ld\n",node, (unsigned long) arrayGet(queues->localQueue[node].individualFlowCounts, packet->flow));
...
t_return * simulationSimulate(t_graph * graph, t_array * paths, t_array * flowTimes, t_array * frameTxDurations) {
	...
	float simTime=  ((float)clock() - t)/((CLOCKS_PER_SEC/1000));
	...
	for (i = 0; i < numberOfFlows; i++) {
		...
		for(int n=0; n < arrayLength(arrayGet(paths, i)); n++){
			node = (long) arrayGet(arrayGet(paths, i), n);
		...
		numberOfLinks += arrayLength(arrayGet(paths, i));
	}
...
//graphPrint(conflict);
	...
	//Constant Queue Size = 10
	queues = queuesNew(graphSize(graph), 10);
	//Dynamic Queue Size
...
//						otherPacket->ETA = 0.025833333 * GRAPH_MULTIPLIER; // First backoff: 15.5 slots.
					...
					if (packet->currentHop == arrayLength(arrayGet(paths, packet->flow)) - 1) {
						...
						//if((time - packet->initialTime)*0.000089>500){
						//	printf("DELAY FLOW %d: %.2f\n",packet->flow, (time - packet->initialTime)*0.000089);		
						//}
...
//							otherPacket->ETA = 0.025833333 * GRAPH_MULTIPLIER; // First backoff: 15.5 slots.
		...
		for (i = 0; i < numberOfFlows; i++) {
			...
			if(scheduleTime <= 0){
				...
				newPacket->flow = i;
				newPacket->deliveryProbability = 1.0;
				//newPacket->ETA = graphGetCost(graph, (long) arrayGet(arrayGet(paths, newPacket->flow), 0), (long) arrayGet(arrayGet(paths, newPacket->flow), 1)); //DOES THAT MAKE SENSE?
				// printf("New Packet Created - Time: %lu Flow: %d\n", time, newPacket->flow);
				queuesAddPacket(queues, newPacket, (long) arrayGet(arrayGet(paths, newPacket->flow), 0));
				if (!arrayGet(backoff, (long) arrayGet(arrayGet(paths, newPacket->flow), 0))) {
					...
					// Fill data regarding backoff. ??
					otherPacket->ETA = (backoffUnit[simulationConflictNodeIndex(linkIndexBase, newPacket->flow, newPacket->currentHop)] 
							* slotTime / ((int) arrayGet(frameTxDurations, newPacket->flow)/1000000.0)) * GRAPH_MULTIPLIER; // First backoff.
					otherPacket->waitingSince = time;
					otherPacket->retries = 0;
					otherPacket->maxRetries = numberOfRetries[simulationConflictNodeIndex(linkIndexBase, otherPacket->flow, otherPacket->currentHop)];
				...
				arraySet(scheduleFlowTime,i, scheduleTime);
			}
			
			if(delta > arrayGet(scheduleFlowTime,i)){
				delta = arrayGet(scheduleFlowTime,i);
				//printf("2.delta %d\n", delta);
			}	
		}
...
//printf("Mean cicle: time %f | delivered packets %f\n", meanTime, meanDelivery);
					...
					for (int j = 0; j < numberOfFlows; j++){
						...
						//****ERRO NESSA ATRIBUIÇÃO ***///
						meanDelayFlows[j] = meanDelayFlows[j] * (1 - alfa)  + (((long) arrayGet(delayFlows,j) - oldDelayFlows[j])/meanDeliveryPerFlow[j] ) * alfa; //Original
						//meanDelayFlows[j] =  1;//(long) arrayGet(delayFlows,j);
						
						//printf("flow: %d - delay: %.2f\n", 0.000089*meanDelayFlows[j] );
				...
				lookAhead++;
//printf("lookAhead: %ld - time %ld\n",lookAhead, time);
//default lookAhead > 30000
/*			if (lookAhead > 30000) {
...
*/
				//heuristic stop criteria time 100ms=100000000; 500ms=500000000; 1000ms=1000000000; 5000ms=5000000000; 10000ms=10000000000; 50000ms=50000000000
				if(time>100000000){
							...
							for (int f = 0; f < numberOfFlows; f++) {
								...
								//r->rateFlows[f]= 8000/(meanInterval*0.000089);
								//r->delayFlows[f]= r->delayFlows[f]*0.000089;
								r->rateFlows[f]=8000/(meanInterval*0.00005497);
						        r->delayFlows[f]= r->delayFlows[f]*0.00005497;
								float delay = r->delayFlows[f];
								r->delay =  r->delay+(delay/300);
								float packetLoss = r->packetsLossPerFlow[f];
								r->packetLoss = r->packetLoss+(packetLoss/numberOfFlows);
								float throughput = r->rateFlows[f];
								r->throughput=r->throughput+throughput;
								r->optimal->packetSentBetweenStates[f] = -1;
								r->optimal->packetDeliveredBetweenStates[f] = -1;
								r->optimal->timeBetweenStates[f] = simTime;//BetweenStates;

								//original versions a and b.
								r->cost= r->cost + 1000* ((meanInterval-flowTime)/meanInterval);
								//version c and d
								//r->cost= r->cost+(100-(packetLoss/numberOfFlows));
								//version e and f
								//r->cost= r->cost+(1/(delay/300));
								//version mix
								//r->cost= r->cost + (1*((meanInterval-flowTime)/meanInterval) + 1*(1-(delay/250)) + 1*(100-(packetLoss/numberOfFlows)));
								 //Estimated regression equation rand -0.01466167943	1.863310653	0.2962983395	-135.6942171
							    // r->cost= r->cost + (100/(0.296*throughput+1.863*packetLoss-0.0146*delay));
				...
				if (deliveredPackets) {
					...
					for (int f = 0; f < numberOfFlows; f++){ 
						...
						if (deliveredPacketsFlowsBetweenStates != 0){
							...
							//printf("f: %ld - delayFlows: %ld, oldState: %ld, deliveredPacketsFlowsBetweenStates: %ld, timeBetweenStates: %ld", (long) arrayGet(delayFlows,f), arrayGet(stateGetDelayFlows(oldState),f), deliveredPacketsFlowsBetweenStates, timeBetweenStates );
						...
						//r->cost= r->cost+(1/(delay/300));
						//version mix
						//r->cost= r->cost + (1*((meanInterval-flowTime)/meanInterval) + 1*(1-(delay/300)) + 1*(100-(packetLoss/numberOfFlows)));
						//Estimated regression equation 0.106090411	5.288545683	0.7384365661	-497.0777159
						 //r->cost= r->cost + (100/(0.296*throughput+1.863*packetLoss-0.0146*delay));
...
//printf("DeliveredPackets = %f, deliveredPacketsBetweenStates = %f\n", deliveredPackets, deliveredPacketsBetweenStates);
					...
					r->packetLoss =GRAPH_INFINITY;
					r->throughput = GRAPH_INFINITY;
					meanInterval = GRAPH_INFINITY;
				}
				stateFree(state);
				free(state);
